(function(){
              try{if(!document.getElementById('dunta-native-fixes')){var s=document.createElement('style');s.id='dunta-native-fixes';s.textContent='html,body{width:100%!important;overflow-x:hidden!important}#dunta-driver-bar{position:fixed;left:12px;right:12px;bottom:18px;z-index:9999;background:#111;border:1px solid #333;border-radius:16px;padding:12px;display:none;color:#fff}#dunta-driver-bar.show{display:block}#dunta-driver-trip{position:fixed;left:12px;right:12px;top:64px;z-index:9998;background:#111827;border-radius:14px;padding:12px;color:#fff;display:none}#dunta-driver-trip.show{display:block}.driver-self-marker{width:42px;height:42px;border-radius:50%;background:#C10F06;border:3px solid #fff;display:grid;place-items:center}.driver-self-marker i{color:#FFD400}body.dunta-driver-mode #home .svc{display:none!important}';document.head.appendChild(s)}}catch(e){}
              ...
              function ensureUI(){
                if(!document.getElementById('dunta-driver-bar')){
                  ...
                  b.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div><b id="db-title">Motorista</b><div id="db-sub" style="font-size:12px;opacity:.75">Fique online</div></div><button id="db-toggle" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#C10F06;color:#fff">Online</button></div>';
                  ...
                  t.innerHTML='<b>Viagem ativa</b><div id="db-trip-info" style="margin-top:6px">—</div><div style="display:flex;gap:8px;margin-top:10px"><button id="db-done" style="flex:1;border:0;border-radius:10px;padding:10px;font-weight:800;background:#16A34A;color:#fff">Concluir</button></div>';
              ...
              function updUI(){
                try{
                  ...
                  if(title)title.textContent=(s.vehicle==='mota'?'Mototaxista':'Motorista')+' · '+(on?'Online':'Offline');
                  if(sub)sub.textContent=on?'A receber pedidos (15 km)':'Toque para ficar online';
                  if(btn){btn.textContent=on?'Offline':'Online';btn.style.background=on?'#16A34A':'#C10F06'}
                  if(window.duntaActiveRide&&window.duntaActiveRide.status==='accepted'){
                    ...
                    if(info)info.innerHTML='Passageiro: <b>'+(window.duntaActiveRide.passenger_name||'—')+'</b><br>Destino: '+(window.duntaActiveRide.destination||'—');
                  }else if(trip)trip.classList.remove('show');
                }catch(e){}
              }
              if(typeof setMe==='function'&&!window.__duntaSetMeFixed){
                window.__duntaSetMeFixed=1;
                var _sm=setMe;
              ...
              if(typeof startDriverSync==='function'&&!window.__duntaLiveFixed){
                ...
                window.startDriverSync=function(){
                  ...
                  publishDriverLocation&&publishDriverLocation();
                  updUI();
                };
              }
              ...
              if(!window.__duntaNativeEnterWrapped&&typeof enterApp==='function'){
                window.__duntaNativeEnterWrapped=1;
                var _en=enterApp;
