...
class MainActivity : AppCompatActivity() {
    ...
    private val permissions = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { result -> if(result[Manifest.permission.ACCESS_FINE_LOCATION]==true || result[Manifest.permission.ACCESS_COARSE_LOCATION]==true) ContextCompat.startForegroundService(this, Intent(this, DriverLocationService::class.java)) }
        ...
        web = WebView(this).apply {
            ...
            webChromeClient=object : WebChromeClient() {
                override fun onGeolocationPermissionsShowPrompt(origin: String?, callback: GeolocationPermissions.Callback?) {
                    if (hasLocationPermission()) callback?.invoke(origin, true, false) else { requestLocationAndNotifications(); callback?.invoke(origin, true, false) }
                }
            }
    ...
    inner class NativeBridge {
        @JavascriptInterface fun setDriverSession(json:String) { try { val o=JSONObject(json); getSharedPreferences("dunta",0).edit { putString("access_token",o.optString("access_token")); putString("refresh_token",o.optString("refresh_token")); putString("user_id",o.optString("user_id")); putString("name",o.optString("name")); putString("phone",o.optString("phone")); putString("vehicle",o.optString("vehicle")) }; registerFcmToken() } catch(_:Exception){} }
        @JavascriptInterface fun startDriverService() { requestLocationAndNotifications(); ContextCompat.startForegroundService(this@MainActivity, Intent(this@MainActivity, DriverLocationService::class.java)) }
        @JavascriptInterface fun stopDriverService() { stopService(Intent(this@MainActivity, DriverLocationService::class.java)) }
    }
}
