...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleNotificationIntent(intent: Intent?) {
        ...
        val fromNotif = intent.getBooleanExtra("from_notification", false) ||
            intent.getBooleanExtra("dunta_open_app", false)
        ...
        web.postDelayed({
            try { startWebGps() } catch (_: Exception) {}
            val rid = (rideId ?: "").replace("'", "").replace("\\", "")
            val t = type.replace("'", "")
            val js = "(function(){try{" +
                "window.__duntaOpenFromPush=true;" +
                "window.__duntaPushRideId='" + rid + "';" +
                "window.__duntaPushType='" + t + "';" +
                "if(typeof startGPS==='function')startGPS();" +
                "if(typeof duntaForceFreshLocation==='function')duntaForceFreshLocation();" +
                "if(typeof window.duntaHandlePushOpen==='function'){window.duntaHandlePushOpen('" + rid + "','" + t + "');}" +
        ...
        web.postDelayed({
            val rid = (rideId ?: "").replace("'", "").replace("\\", "")
            val t = type.replace("'", "")
            val js2 = "(function(){try{if(typeof window.duntaHandlePushOpen==='function'){window.duntaHandlePushOpen('" + rid + "','" + t + "');}if(typeof duntaForceFreshLocation==='function')duntaForceFreshLocation();}catch(e){}})();"
            web.evaluateJavascript(js2, null)
        }, 1800)
    }
    ...
    private fun injectDuntaFixes(view: WebView) {
        ...
        try {
            val live = assets.open("dunta-live.js").bufferedReader(Charsets.UTF_8).use { it.readText() }
        ...
        try {
            val gps = assets.open("dunta-gps.js").bufferedReader(Charsets.UTF_8).use { it.readText() }
            view.evaluateJavascript(gps, null)
        } catch (e: Exception) { e.printStackTrace() }
    }
