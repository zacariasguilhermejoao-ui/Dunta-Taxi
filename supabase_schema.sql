...
-- Preenche o perfil automaticamente quando uma conta Auth é criada.
...
begin
  ...
  values (
    new.id,
    coalesce(new.raw_user_meta_data->>'name','Utilizador DUNTA'),
    coalesce(new.raw_user_meta_data->>'phone',new.email),
    coalesce(new.raw_user_meta_data->>'role','passenger'),
    new.raw_user_meta_data->>'vehicle',
    new.raw_user_meta_data->>'vehicle_id'
  )
  on conflict (id) do update set
    name=excluded.name, phone=excluded.phone, role=excluded.role,
    vehicle=excluded.vehicle, vehicle_id=excluded.vehicle_id, updated_at=now();
  return new;
end $$;
...
create policy "ride driver accept" on public.ride_requests for update using (status='pending' and exists(select 1 from public.profiles p where p.id=auth.uid() and p.role='driver'))
with check (driver_id=auth.uid());

-- Realtime
do $$ begin
  alter publication supabase_realtime add table public.drivers_locations;
...
-- Remove motoristas presos como online por mais de 60 segundos.
update public.drivers_locations
set is_online=false
where is_online=true and updated_at < now() - interval '60 seconds';

-- Nota: no Supabase Dashboard, Authentication > Providers > Email:
-- desative "Confirm email" se quiser login imediato sem validação de email.

-- ============================================================
-- PERFIS COM FOTO + STORAGE DE AVATARS
-- Execute esta parte também se já executou o schema anterior.
-- ============================================================
...
create policy "avatars authenticated upload own folder" on storage.objects
for insert to authenticated
with check (bucket_id='avatars' and (storage.foldername(name))[1]=auth.uid()::text);

drop policy if exists "avatars authenticated update own folder" on storage.objects;
create policy "avatars authenticated update own folder" on storage.objects
for update to authenticated
using (bucket_id='avatars' and (storage.foldername(name))[1]=auth.uid()::text)
with check (bucket_id='avatars' and (storage.foldername(name))[1]=auth.uid()::text);

drop policy if exists "avatars authenticated delete own folder" on storage.objects;
create policy "avatars authenticated delete own folder" on storage.objects
for delete to authenticated
using (bucket_id='avatars' and (storage.foldername(name))[1]=auth.uid()::text);

-- O trigger passa também a guardar avatar_url vindo dos metadados quando existir.
create or replace function public.handle_new_user()
returns trigger language plpgsql security definer set search_path=public
as $$
begin
  ...
  on conflict (id) do update set
    name=excluded.name, phone=excluded.phone, role=excluded.role,
    vehicle=excluded.vehicle, vehicle_id=excluded.vehicle_id,
    avatar_url=coalesce(excluded.avatar_url,public.profiles.avatar_url), updated_at=now();
  return new;
end $$;

-- Backfill simples: mantém drivers_locations sincronizados quando a foto muda.
create or replace function public.sync_profile_avatar()
returns trigger language plpgsql security definer set search_path=public
as $$
begin
  update public.drivers_locations set avatar_url=new.avatar_url where driver_id=new.id;
  return new;
end $$;

drop trigger if exists sync_profile_avatar_after_update on public.profiles;
create trigger sync_profile_avatar_after_update
after update of avatar_url on public.profiles
for each row execute procedure public.sync_profile_avatar();

-- Para Realtime receber os campos novos, as tabelas já estão na publicação.

-- FCM: tokens nativos Android para push real
...
-- ============================================================
-- PUSH SERVER-SIDE
-- O envio FCM é feito pela Edge Function send-ride-push.
-- Configure um Database Webhook no Supabase para chamar a função
-- em INSERT/UPDATE de public.ride_requests.
-- ============================================================
create index if not exists idx_push_tokens_user_enabled
  on public.push_tokens(user_id, enabled, platform);
